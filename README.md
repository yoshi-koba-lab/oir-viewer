...
- **Plate** — オリンパス MATL 撮影のフォルダを読み、`matl.omp2info` から
  ...
  独立した2つの情報から二重に検証します。選んだウェルの 3D 表示を、**プレート
  と同じ配置のまま 1 枚の PDF** に書き出せます。詳しくは下記。
...
**Plate** でフォルダを読み、ウェルを選んで **PDF を作成** を押すと、各ウェルの 3D
表示がプレートと同じ行・列に並んだ PDF が出ます。ウェルは 1 つずつ順に処理され、
次を読む前に前のウェルを解放します。

**コントラストは画面で設定したものをそのまま焼き込みます。** ウェルごとの自動調整は
しません。同じ条件で撮ったウェルどうしを PDF の上で比較するには、明るさの調整が
ウェルごとに変わっては困るためです。表示中のチャンネル（最大 4 つ）だけが描かれます。
