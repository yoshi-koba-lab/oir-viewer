 ...
 */
export function PlateTable({ wells, disabled = false }: { wells: OpenWell[]; disabled?: boolean }) {
  ...
  const startRename = (c: PlateColumn) => {
    if (disabled) return;
    setRenaming(c.key);
    setDraft(c.label);
  };
  ...
  if (wells.length === 0) {
    return (
      <p className="text-[11px] text-[var(--text-secondary)] leading-relaxed">
        開いているウェルがありません。ツールバーの Plate でウェルを開き、
              ...
              {columns.map((c, i) => (
                <th key={c.key} className="border-b border-r border-[var(--border)] px-1 py-1
                  ...
                  {renaming === c.key ? (
                    ...
                    <button onClick={() => startRename(c)} disabled={disabled}
                            title="クリックで列名を変更"
                  ...
                  <div className="flex items-center gap-0.5 mt-0.5 font-normal">
                    ...
                    >図</button>
                    <button onClick={() => moveColumn(c.key, -1)} disabled={disabled || i === 0}
                            title="左へ"
                            className="px-0.5 text-[var(--text-secondary)] hover:text-white disabled:opacity-25">‹</button>
                    <button onClick={() => moveColumn(c.key, 1)} disabled={disabled || i === columns.length - 1}
                            title="右へ"
                            className="px-0.5 text-[var(--text-secondary)] hover:text-white disabled:opacity-25">›</button>
                    <button onClick={() => removeColumn(c.key)} disabled={disabled}
                            title="この列を削除"
                            className="px-0.5 text-[var(--text-secondary)] hover:text-red-400
                                       disabled:opacity-25 disabled:hover:text-[var(--text-secondary)]">×</button>
                  </div>
                </th>
              ))}
            </tr>
